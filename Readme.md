...
To understand where the formulas come from, let's have a look at a simplified $2d \rightarrow 1d$ projection example. Consider the two **similar triangles**:
   ...
   - From here it is clear that $\frac{x'}{x} = \frac{y'}{y}$ holds, which leads under the assumption of $x' = 1$ to the projection formulas $y' = \frac{y}{x}$. With this calculation in mind the above projection formulas for $x_{proj} and y_{proj}$ become clear.
